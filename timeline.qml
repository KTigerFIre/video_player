...
Rectangle {
    ...
    SystemPalette {
        id: activePalette
        property color scrollBar: (windowText.hsvValue
                                   > window.hsvValue) ? Qt.lighter(
                                                            window) : Qt.darker(
                                                            window)
    }
    ...
    Row {
                ...
                Column {
                    ...
                    Repeater {
                        ...
                        TrackHead {
                            ...
                            onIsLockedChanged: tracksRepeater.itemAt(
                            ...
                            MouseArea {
                                ...
                                width: containsMouse
                                       | drag.active ? Math.max(
                                                           parent.width,
                                                           dragItemText.contentWidth + 20) : 8
                                onReleased: {
                                    if (drag.active)
                                        dragItem.Drag.drop()
                                }
                                ...
                                Rectangle {
                                    ...
                                    opacity: dragMouseArea.containsMouse
                                             | dragMouseArea.drag.active ? 0.7 : 0
                                    states: [
                                        State {
                                            when: dragMouseArea.drag.active
                                                  | dragMouseArea.containsMouse
                            ...
                            DropArea {
                                ...
                                keys: ["trackHeader"]
                                onEntered: drag => {
                                               if (trackHead.isVideo
                                                   === drag.source.trackHead.isVideo) {
                                ...
                                onDropped: drop => {
                                               if (drop.proposedAction === Qt.MoveAction) {
                                                   if (trackHead.isVideo
                                                       && !drop.source.trackHead.isVideo) {
                                                       application.showStatusMessage(
                                                           qsTr('Can not move audio track above video track'))
                                                   } else if (!trackHead.isVideo
                                                              && drop.source.trackHead.isVideo) {
                                                       application.showStatusMessage(
                                                           qsTr('Can not move video track below audio track'))
                                                   } else if (trackHead.trackIndex === drop.source.trackHead.trackIndex) {
                                                       application.showStatusMessage(
                                                           qsTr('Track %1 was not moved').arg(
                                                               drop.source.trackHead.trackName))
                                                   } else {
                                                       drop.acceptProposedAction()
                                                       timeline.moveTrack(
                                                           drop.source.trackHead.trackIndex,
                                                           trackHead.trackIndex)
                                                   }
                                               }
                                               containsValidDrag = false
                                           }
        ...
        Item {
            ...
            MouseArea {
                ...
                hoverEnabled: true
                onClicked: mouse => {
                               timeline.position = (tracksFlickable.contentX + mouse.x)
                               / multitrack.scaleFactor
                               bubbleHelp.hide()
                           }
                ...
                onPositionChanged: mouse => {
                                       if (mouse.modifiers === (Qt.ShiftModifier | Qt.AltModifier)
            ...
            MouseArea {
                ...
                onPositionChanged: {
                    ...
                    startX = mouse.x
                    tracksFlickable.contentX = Logic.clamp(
                                tracksFlickable.contentX - n, 0,
                                Logic.scrollMax().x)
                    n = mouse.y - startY
                    startY = mouse.y
                    tracksFlickable.contentY = Logic.clamp(
                                tracksFlickable.contentY - n, 0,
                                Logic.scrollMax().y)
                }
            }
                    ...
                    Item {
                        ...
                        Item {
                            ...
                            Repeater {
                                ...
                                Rectangle {
                                    property var clip: trackAt(
                                                           modelData.y).clipAt(
                                                           modelData.x)
                                    property var track: typeof clip !== 'undefined'
                                                        && typeof dragDelta !== 'undefined' ? trackAt(clip.trackIndex + dragDelta.y) : 0

                                    x: clip && typeof dragDelta
                                       !== 'undefined' ? clip.x + dragDelta.x : 0
                                    y: track ? track.y : 0
                                    width: clip ? clip.width : 0
                                    height: track ? track.height : 0
                                    color: 'transparent'
                                    border.color: 'red'
                                    visible: clip && !clip.Drag.active
            ...
            CornerSelectionShadow {
                y: tracksRepeater.count ? tracksRepeater.itemAt(
                                              timeline.currentTrack).y + ruler.height
                                          - tracksFlickable.contentY : 0
                clip: timeline.selection.length ? tracksRepeater.itemAt(
                                                      timeline.selection[0].y).clipAt(
                                                      timeline.selection[0].x) : null
                opacity: clip
                         && clip.x + clip.width < tracksFlickable.contentX ? 1 : 0
            }
            ...
            CornerSelectionShadow {
                y: tracksRepeater.count ? tracksRepeater.itemAt(
                                          ...
                                          - tracksFlickable.contentY : 0
                clip: timeline.selection.length ? tracksRepeater.itemAt(
                                                      timeline.selection[timeline.selection.length - 1].y).clipAt(timeline.selection[timeline.selection.length - 1].x) : null
                opacity: clip
                         && clip.x > tracksFlickable.contentX + tracksFlickable.width ? 1 : 0
                anchors.right: parent.right
                mirrorGradient: true
            }
        ...
        Track {
            ...
            onClipClicked: (clip, track, mouse) => {
                               ...
                               if (timeline.selection.length === 1
                                   && tracksRepeater.itemAt(
                                       timeline.selection[0].y).clipAt(
                                       timeline.selection[0].x).isBlank)
                               timeline.selection = []
                               if (tracksRepeater.itemAt(trackIndex).clipAt(
                                       clipIndex).isBlank)
                               timeline.selection = [Qt.point(clipIndex,
                                                              trackIndex)]
                               else if (mouse
                                        && mouse.modifiers & Qt.ControlModifier)
                               timeline.selection = Logic.toggleSelection(
                                   trackIndex, clipIndex)
                               else if (mouse
                                        && mouse.modifiers & Qt.ShiftModifier)
                               timeline.selection = Logic.selectRange(
                                   trackIndex, clipIndex)
                               else if (!Logic.selectionContains(trackIndex,
                                                                 clipIndex))
                               timeline.selection = [Qt.point(clipIndex,
                                                              trackIndex)]
                               root.clipClicked()
    ...
    Connections {
        ...
        function onZoomToFit() {
            setZoom(Math.pow(
                        (tracksFlickable.width - 50) * multitrack.scaleFactor
                        / tracksContainer.width - 0.01, 1 / 3))
